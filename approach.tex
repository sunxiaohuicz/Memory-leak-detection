...
%Step2: (Fold paths)\\
3.\quad	\textbf{do while} $\mathit{Red\_Black\_Tree}$.hasNext\\
4.\quad	\textbf{begin}\\
5.\quad	\textbf{if} $n.ind\geq 2 || n.outd\geq 2$ \textbf{then}\\
6.\quad \quad    \textbf{if} $A.a\in \Gamma(n) || F.f\in \Gamma(n)$\\
7.\quad \quad	    \textbf{then} $\Gamma(n)\leftarrow A.a || \Gamma(n)\leftarrow F.f$\\
8.\quad	\textbf{else} \\
9.\quad \quad   \textbf{then} $\Gamma(n)\leftarrow (n\cup \Gamma(n)\cap N^*)$\\
10.\quad	\textbf{end}\\
%Step3: (Adjust the $\mathit{Red\_Black\_Tree}(N^*)$ and the $\mathit{Map}(N^*)$, and obtain the final projection graph $G^*$)\\
11.\quad	\textbf{delete} $n (n\not\in N^*)$;\\
12.\quad	$\mathit{Red\_Black\_Tree(N)} \leftarrow \mathit{Red\_Black\_Tree}(N^*)$\\
13.\quad	$\mathit{Map}(N) \leftarrow \mathit{Map}(N^*)$;\\
%\textbf{End}
\end{algorithm}
