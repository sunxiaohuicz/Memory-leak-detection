...
%Similar to the CFG based static analysis, this paper also first constructs a detection model of C program. The detection model contains different kinds of memory leaks. The main difference from our approach is that, 
...
Fig.~\ref{fig:4} shows the projection graph $(G^*)$ of $G$. Following the projection definition, we have $N^*=A\cup B\cup F=\{S_1, S_3, S_6, S_7, S_9\}$. Similarly, we have the edges from $S_1$ to $S_3$, $S_3$ to $S_6$, $S_6$ to $S_7$ and $S_7$ to $S_9$. Apart from this, $S_3$ is a closed node since all its direct successor nodes are not memory related nodes, thus we add a loop to $S_3$. There is no loop at $S_6$, because the direct successor $s_7$ is a memory related node.  
...
Algorithm~\ref{alg:A1} shows the projection algorithm. %We can summarize from the control flow graph projection theorem that $G^*$ is a directed graph extracted from $G$ which consists of $N^*$. Apart from this, $G^*$ strictly keeps the reachable relationships among nodes in $N^*$ from $G$. Specifically, if $N^* = N$, then $P[G, N^*] = G$.
%
